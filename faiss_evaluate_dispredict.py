...
def get_args():
    ...
    parser.add_argument('--rerank_model_name_or_path', default="BAAI/bge-base-en", type=str)
    ...
    parser.add_argument('--rerank_k', default=10, type=int)
...
def get_rerank_model(model_path, device):
...
def evaluate(args):
    rerank_model = get_rerank_model(args.rerank_model_name_or_path, args.device)
    ...
    for file in os.listdir(args.model_name_or_path):
        if file.startswith("checkpoint"):
            ...
            res_scores, res_index = batch_search(index, q_vecs, topk=args.top_k+args.rerank_k, batch_size=64)

            
            res_index_new = []
            for query, res in tqdm(zip(queries, res_index), total=len(queries), desc="Reranking"):
                passage = [corpus[id] for id in res]
                ranks = rerank_model.rank(query, passage)

                new_res = [res[rank['corpus_id']] for rank in ranks]
                res_index_new.append(new_res)
            res_index = res_index_new
            ...
            with open('xxxxx.jsonl', 'w', encoding='utf-8') as output_file:
                for res, data in zip(res_index, eval_dataset):
                    ...
                    new_data = {
                        "diag_list": disease_list
                    }
                    json.dump(new_data, output_file, ensure_ascii=False)
                    output_file.write('\n')
                    
                    
                    for dis in disease_list[:args.top_k]:
